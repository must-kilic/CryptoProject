// aesManualCore.js — Kütüphanesiz AES-128

// --- S-Box ---
export const SBOX = [
    
];

export const RCON = [
    0x00, 0x01, 0x02, 0x04, 0x08,
    0x10, 0x20, 0x40, 0x80,
    0x1B, 0x36
];

// GF multiplication for MixColumns
function xtime(x) {
    return ((x << 1) ^ (((x >> 7) & 1) * 0x1b)) & 0xFF;
}

export function mixColumns(state) {
    for (let i = 0; i < 4; i++) {
        const a = state[0][i], b = state[1][i], c = state[2][i], d = state[3][i];
        state[0][i] = xtime(a) ^ xtime(b) ^ b ^ c ^ d;
        state[1][i] = a ^ xtime(b) ^ xtime(c) ^ c ^ d;
        state[2][i] = a ^ b ^ xtime(c) ^ xtime(d) ^ d;
        state[3][i] = xtime(a) ^ a ^ b ^ c ^ xtime(d);
    }
}

export function subBytes(state) {
    for (let r = 0; r < 4; r++)
        for (let c = 0; c < 4; c++)
            state[r][c] = SBOX[state[r][c]];
}

export function shiftRows(state) {
    state[1].push(state[1].shift());
    state[2].push(state[2].shift(), state[2].shift());
    state[3].unshift(state[3].pop());
}

export function addRoundKey(state, roundKey) {
    for (let r = 0; r < 4; r++)
        for (let c = 0; c < 4; c++)
            state[r][c] ^= roundKey[r][c];
}

export function keyExpansion(keyBytes) {
    const keyWords = [];
    for (let i = 0; i < 4; i++)
        keyWords[i] = keyBytes.slice(i * 4, i * 4 + 4);

    const expanded = [...keyWords];
    let i = 4;

    while (expanded.length < 44) {
        let temp = expanded[i - 1];

        if (i % 4 === 0) {
            temp = rotWord(temp);
            temp = subWord(temp);
            temp[0] ^= RCON[i / 4];
        }

        const newWord = expanded[i - 4].map((b, j) => b ^ temp[j]);
        expanded.push(newWord);
        i++;
    }

    return expanded.map((w, idx) => {
        const r = Math.floor(idx / 4);
        const c = idx % 4;
        return w.map(x => x);
    });
}

function rotWord(word) {
    return word = Uint8Array.from([
        ...word.slice(1),
        word[0]
    ]);

}

function subWord(word) {
    return word.map(v => SBOX[v]);
}

export function bytesToState(bytes) {
    return [
        [bytes[0], bytes[4], bytes[8], bytes[12]],
        [bytes[1], bytes[5], bytes[9], bytes[13]],
        [bytes[2], bytes[6], bytes[10], bytes[14]],
        [bytes[3], bytes[7], bytes[11], bytes[15]]
    ];
}

export function stateToBytes(state) {
    return [
        state[0][0], state[1][0], state[2][0], state[3][0],
        state[0][1], state[1][1], state[2][1], state[3][1],
        state[0][2], state[1][2], state[2][2], state[3][2],
        state[0][3], state[1][3], state[2][3], state[3][3]
    ];
}
